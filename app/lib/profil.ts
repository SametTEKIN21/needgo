 ...
 * Mobil `needgoapp/src/lib/profil.ts` ile aynı sözleşme.
 */
export async function profilGetir(): Promise<Profil> {
  const { data, error } = await supabase.rpc('profil_getir')
  if (error || !data) return BOS_PROFIL
  return { ...BOS_PROFIL, ...(data as Partial<Profil>) }
}
...
export async function hesabiSil(): Promise<void> {
  ...
  try {
    ...
    // sunucudaki oturum zaten geçersiz
  }
}
